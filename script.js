function init () {
  ...
  document.getElementById("terminale").input class="button" type="submit" value="Voir le résultat!" = resultat;
  ...
  if(sReq !=0)
  {
      ...
      document.getElementById('resultat').mess = resultat;
  }
}
...
function resultat() {
  ...
  // Vérifier si le classe a été saisie. Si non (chaine vide), ajouter "\n - Classe" à message
  ...
  // Vérifier si les deux spécialité ont été saisie. Si non (chaine vide), ajouter "\n - Deux spécialités" à message
  if(cpt!=2){
    message += "\n- Deux spécialités";
  }
